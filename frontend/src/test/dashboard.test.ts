...
describe("Phase 16 dashboard contract", () => {
  it("keeps the MITRE coverage calculation as a backend-provided value", () => {
    const payload = { total_techniques: 15, mapped_techniques: 3, coverage_percent: 20 };
    expect(payload.coverage_percent).toBe(20);
    expect(payload.mapped_techniques).toBeLessThanOrEqual(payload.total_techniques);
  });
